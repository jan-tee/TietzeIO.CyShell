...
namespace TietzeIO.CyShell.Cmdlets.Base
{
    ...
    /// be string.</typeparam>
    public abstract class CyParallelPipelineCmdlet<TOutput, TWorkItem> : CyApiCmdlet where TOutput : new()
    {
        ...
        protected CyParallelPipelineCmdlet() : base()
        {
        ...
        protected void QueueObject(TWorkItem o)
        {
            if (!Stopping)
            {
                ...
                if (InputObjectsQueue.Count == ObjectsPerTransaction)
                {
                    WriteVerbose($"High water threshold of {ObjectsPerTransaction} reached, creating task. Object queue length: {InputObjectsQueue.Count}. Task queue length: {TasksQueue.Count}");
                ...
                while ((TasksQueue.Count > 0) && (TasksQueue.Peek().IsCompleted))
                {
                    ...
                    switch (task.Status)
                    {
                        ...
                        case TaskStatus.Faulted:
                            if (EnableOutput)
                            {
                                WriteError(new System.Management.Automation.ErrorRecord(task.Exception?.InnerException, "TaskFaulted", System.Management.Automation.ErrorCategory.OperationStopped, null));
                            }
                            break;
                        case TaskStatus.Canceled:
                            if (EnableOutput)
                            {
                                WriteError(new System.Management.Automation.ErrorRecord(task.Exception?.InnerException, "TaskCanceled", System.Management.Automation.ErrorCategory.OperationStopped, null));
                            }
                            break;
                    }
                }
